...
log('training…');
await page.waitForFunction(() => { const b = document.querySelector('button.toolbar-btn-test'); return b && !b.disabled; }, { timeout: 300000, polling: 1000 });
log('trained');
await sleep(800);
...
// drag the weight slider toward the left (higher loss) to move the prediction
await page.evaluate(() => {
  const el = document.querySelector('.bp-wiggle-slider input[type=range]');
  if (!el) return;
  const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
  const lo = parseFloat(el.min), hi = parseFloat(el.max);
  setter.call(el, String(lo + (hi - lo) * 0.18));
...
await sleep(400);
const box = await page.$eval('.bp-wiggle', (el) => {
  const r = el.getBoundingClientRect();
  return { x: Math.max(0, r.x - 8), y: Math.max(0, r.y - 8), width: Math.min(1590, r.width + 16), height: Math.min(1040, r.height + 16) };
});
await page.screenshot({ path: '/tmp/wg-dragged.png', clip: box });
const readout = await page.$eval('.bp-wiggle-slider', (el) => {
  const r = el.getBoundingClientRect();
  return { x: Math.max(0, r.x - 10), y: Math.max(0, r.y - 12), width: Math.min(1590, r.width + 20), height: r.height + 40 };
});
await page.screenshot({ path: '/tmp/wg-readout.png', clip: readout });
log('saved', JSON.stringify(readout));
await browser.close();
