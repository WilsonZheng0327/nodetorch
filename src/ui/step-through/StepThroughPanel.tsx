...
export function StepThroughPanel({ open, graphJson, onClose }: Props) {
    ...
    if (isGANGraph) {
      ...
      fetch(apiUrl('/denoise-step-through'), {
        ...
        body: JSON.stringify({
          ...
          numSamples: 4,
          captureEvery: Math.max(1, Math.floor(100 / 50)),
        }),
      })
        .then((r) => r.json())
        .then((data) => {
  ...
  const loadGenerate = useCallback(() => {
    ...
    fetch(apiUrl('/generate-text'), {
      ...
      .then((r) => r.json())
      .then((data) => {
        if (data.status === 'ok') setGenResult(data.result);
        else setError(data.error ?? 'Failed to generate text');
      })
      .catch(() => setError('Cannot connect to backend'))
      .finally(() => setLoading(false));
  ...
  const isDiffusionGraph = (() => {
    try {
      return (
        JSON.parse(graphJson).graph?.nodes?.some(
          (n: any) => n.type === 'ml.diffusion.noise_scheduler',
        ) ?? false
      );
    } catch {
      return false;
  ...
  const isGANGraph = (() => {
    try {
      return (
        JSON.parse(graphJson).graph?.nodes?.some(
          (n: any) => n.type === 'ml.gan.noise_input' || n.type === 'ml.loss.gan',
        ) ?? false
      );
    } catch {
      return false;
  ...
  const isAutoregGraph = (() => {
    try {
      return (
        JSON.parse(graphJson).graph?.nodes?.some((n: any) => n.type === 'data.tiny_shakespeare') ??
        false
      );
    } catch {
      return false;
      ...
      <div className="generate-controls">
        ...
        <div className="generate-sliders">
          <div className="generate-slider-group">
            <label className="generate-label" title="Controls randomness.">
              Temperature <span className="generate-label-help">?</span>
            </label>
            <div className="generate-slider-row">
              ...
              <input
                type="number"
                className="generate-slider-value"
                min={0.1}
                max={2.0}
                step={0.05}
                value={temperature}
                onChange={(e) => {
                  ...
                  if (!isNaN(v)) onTempChange(Math.max(0.1, Math.min(2.0, v)));
                }}
              />
            </div>
          </div>
          <div className="generate-slider-group">
            <label className="generate-label" title="Limits to K most likely tokens.">
            ...
            <div className="generate-slider-row">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(v)) onTopKChange(Math.max(0, Math.min(65, v)));
                }}
              />
            </div>
          </div>
          <div className="generate-slider-group">
            <label className="generate-label" title="Max tokens to generate.">
              Max tokens <span className="generate-label-help">?</span>
            </label>
            <div className="generate-slider-row">
              <input
                type="range"
              ...
              <input
                ...
                onChange={(e) => {
                  const v = parseInt(e.target.value, 10);
                  if (!isNaN(v)) onMaxTokensChange(Math.max(20, Math.min(500, v)));
                }}
              />
            </div>
          </div>
...
function DenoiseView({
  step,
  channels,
}: {
  step: { timestep: number; pixels: (number[][] | number[][][])[] };
  channels: number;
}) {
  return (
    <div className="denoise-view">
      <div className="denoise-info">
        ...
        <span className="denoise-hint">
          {step.timestep > 0 ? 'Denoising in progress' : 'Fully denoised'}
        </span>
      </div>
