...
export const conv1dNode: NodeDefinition = {
  ...
  learnMore:
    'The 1D version of Conv2d \u2014 slides filters along a sequence (text, audio, time series) instead of an image. Useful for detecting local patterns in sequential data without the overhead of recurrent networks.',
      ...
      execute: async ({ inputs, properties }) => {
        ...
        if (outL <= 0) {
          return {
            outputs: {},
            metadata: {
              error: `Invalid output length: ${outL} (kernel ${K} too large for length ${L})`,
            },
          };
        }
