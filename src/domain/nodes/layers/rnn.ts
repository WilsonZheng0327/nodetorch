...
export const rnnNode: NodeDefinition = {
  ...
  learnMore:
    'The simplest recurrent layer \u2014 processes sequences by passing a hidden state from one timestep to the next. Struggles with long sequences due to vanishing gradients. In practice, LSTM or GRU are almost always preferred.',
    ...
    shape: {
      execute: async ({ inputs, properties }) => {
        ...
        if (!input || input.length !== 3) {
          return input
            ? { outputs: {}, metadata: { error: 'RNN expects [batch, seq_len, input_size]' } }
            : { outputs: {} };
        }
